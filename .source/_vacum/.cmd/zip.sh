...
[ -f "$root/.source/_secrets/secrets.age" ] || { echo 'secrets.age ausente' >&2; exit 1; }
cd "$root"
zip -q -r "$out" .source/_vacum .source/_main .source/_secrets/secrets.age .source/_secrets/example .docs \
